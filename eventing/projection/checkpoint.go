...
type ICheckpointStore interface {
	...
	// 返回：
	//   - *Checkpoint: 检查点数据
	//   - error: errors.NotFound 表示检查点不存在，其他错误表示存储失败
...
// ICheckpointTxSessionRequirer 表示检查点存储要求 ctx 中存在 ORM 事务 session。
//
// 典型场景是 SQL checkpoint store 需要与读模型写入共享同一提交边界。
// 若调用方为 store 包了一层装饰器，装饰器应继续透传该能力声明，避免事务保护被绕过。
type ICheckpointTxSessionRequirer interface {
	// RequiresORMTxSession 返回当前 store 是否要求 ctx 中存在 ORM 事务 session。
	RequiresORMTxSession() bool
}

// NewCheckpoint 创建Checkpoint。
