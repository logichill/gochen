...
// NewEventCountStrategy 创建一个按事件版本频率触发的快照策略。
...
// ShouldCreateSnapshot 在聚合版本达到频率倍数时返回 true。
...
func (s *EventCountStrategy[ID]) Name() string {
...
// NewTimeDurationStrategy 创建一个按时间间隔触发的快照策略。
...
// ShouldCreateSnapshot 在距离上次快照超过阈值时返回 true。
func (s *TimeDurationStrategy[ID]) ShouldCreateSnapshot(ctx context.Context, aggregate ISnapshotAggregate[ID]) (bool, error) {
	...
	// 尝试从快照存储获取最后快照时间
	s.mutex.Lock()
	if snapshot, err := s.snapshotStore.FindSnapshot(ctx, aggregateType, aggregateID); err == nil {
...
func (s *TimeDurationStrategy[ID]) Name() string {
	return "TimeDurationStrategy"
}

// UpdateLastSnapshotTime 手动更新内存中的最近快照时间缓存。
...
const defaultAggregateSizeMaxEvents = 1000

// NewAggregateSizeStrategy 创建一个按聚合大小阈值触发的快照策略。
func NewAggregateSizeStrategy[ID comparable](maxEvents int, maxSizeBytes int) *AggregateSizeStrategy[ID] {
	if maxEvents <= 0 {
		maxEvents = defaultAggregateSizeMaxEvents // 默认1000个事件
	}
	if maxSizeBytes <= 0 {
...
// ShouldCreateSnapshot 在事件数或估算大小超过阈值时返回 true。
...
func (s *AggregateSizeStrategy[ID]) Name() string {
...
// NewCompositeSnapshotStrategy 创建一个组合多个子策略的快照策略。
...
// ShouldCreateSnapshot 按组合模式聚合各子策略的判断结果。
...
func (s *CompositeSnapshotStrategy[ID]) Name() string {
	return "CompositeSnapshotStrategy"
}

func (s *CompositeSnapshotStrategy[ID]) Strategies() []ISnapshotStrategy[ID] {
	return s.strategies
}

func (s *CompositeSnapshotStrategy[ID]) Mode() CompositeMode {
	return s.mode
}
