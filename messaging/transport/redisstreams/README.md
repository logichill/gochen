...
4. 将该 Transport 传给 `messaging.NewMessageBus` / `commanding.NewCommandBus` 即可。
...
func NewTransport(cfg Config) (*Transport, error) {
	...
	if cfg.ConsumerName == "" {
		...
		cfg.ConsumerName = fmt.Sprintf("consumer-%d", id)
	}
	...
	if cfg.Logger == nil {
		cfg.Logger = logging.ComponentLogger("transport.redisstreams")
	}
...
func (t *Transport) Subscribe(ctx context.Context, messageType string, handler messaging.IMessageHandler) (messaging.UnsubscribeFunc, error) {
	t.mu.Lock()
	defer t.mu.Unlock()
	...
	return func(unsubCtx context.Context) error {
		if unsubCtx == nil {
			unsubCtx = context.Background()
		}
		var err error
		once.Do(func() { err = t.unsubscribeLocked(unsubCtx, messageType, handler) })
		return err
	}, nil
}

func (t *Transport) unsubscribeLocked(ctx context.Context, messageType string, handler messaging.IMessageHandler) error {
...
// Stop stops consumers and optionally closes the redis client.
...
func encodeMessage(msg messaging.IMessage) (map[string]any, error) {
	...
	aggID, _ := msg.GetMetadata().GetString("aggregate_id")
	aggType, _ := msg.GetMetadata().GetString("aggregate_type")
	...
	return map[string]any{
		...
		"payload":   string(data),
		"aggregate": fmt.Sprintf("%s:%s", aggID, aggType),
	}, nil
}
